...
sys = System(joinpath(pwd(), "ThreeBusInverter.raw"))

slack_bus = first(get_components(x -> get_bustype(x) == BusTypes.REF, Bus, sys))
...
to_json(sys, joinpath(pwd(), "threebus_sys.json"), force = true)